...
async def get_patients(
    ...
    result = await db.execute(
        select(Patient).offset(skip).limit(limit).order_by(Patient.id)
    )
...
async def get_patients_cached(
    ...
    try:
        ...
        serialized = [
            PatientSchema.model_validate(p).model_dump(mode="json") for p in patients
        ]
...
async def get_vaccination(
    db: AsyncSession, vaccination_id: int
...
async def get_all_vaccinations(
    ...
    result = await db.execute(
        select(Vaccination).offset(skip).limit(limit).order_by(Vaccination.id)
    )
    return result.scalars().all()


async def create_vaccination(db: AsyncSession, vacc: VaccinationCreate) -> Vaccination:
    ...
    try:
        ...
        event = {
            ...
            "next_dose_date": (
                db_vacc.next_dose_date.isoformat() if db_vacc.next_dose_date else None
            ),
...
async def update_vaccination(
    db: AsyncSession, vaccination_id: int, vacc_update: VaccinationCreate
) -> Optional[Vaccination]:
...
async def delete_vaccination(db: AsyncSession, vaccination_id: int) -> bool:
    """Удалить вакцинацию"""
    db_vacc = await get_vaccination(db, vaccination_id)
    if not db_vacc:
        return False
